...
@section('content')
    ...
    <div class="ga-wrapper container">
        <h1 class="ga-title">Modifier un avion</h1>
        ...
        <form action="{{ route('avions.update', $avion) }}" method="POST">
            @csrf
            @method('PUT')

            <div class="ga-form-group">
                <label for="modele">Modele</label>
                <input id="modele" type="text" name="modele" value="{{ old('modele', $avion->modele) }}" required>
                @error('modele')
            ...
            <div class="ga-form-group">
                ...
                <input id="capacite" type="number" name="capacite" value="{{ old('capacite', $avion->capacite) }}" min="1" required>
                @error('capacite')
                    <p class="ga-error">{{ $message }}</p>
                @enderror
            </div>
